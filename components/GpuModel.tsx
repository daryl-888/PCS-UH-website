...
/** World-space length the card's longest axis is normalized to, i.e. the one
 *  knob for the card's apparent size. It only means anything relative to the
 *  camera dolly (POSE_ZOOM, ~300–465 units back) and the lateral drift
 *  (POSE_TX/TY/TZ), which are expressed in these same world units — so change
 *  this before reaching for the camera when the card reads too big or small. */
const FIT_MAX_DIM = 320;
...
function GpuCard({
  ...
  // Fit the model into the FIT_MAX_DIM frame, center it, and punch up its
  ...
  //
  // Everything here mutates the GLTF in place, and `useGLTF` hands out the
  // SAME cached `scene` object to every mount (navigating back to "/", Fast
  // Refresh, StrictMode's double render in dev). So each step has to be
  // idempotent — measuring a card that a previous mount already fitted
  // computed FIT_MAX_DIM / FIT_MAX_DIM = scale 1, and multiplying an already
  // reduced roughness by 0.55 again walked it down to the floor.
  useMemo(() => {
    // Back to the GLB's authored transform, so the measurement below is of
    // the raw model rather than of whatever a previous mount left behind.
    ...
    scene.scale.setScalar(FIT_MAX_DIM / maxDim);
    ...
    scene.traverse((obj) => {
      ...
      for (const mat of materials) {
        if (mat instanceof THREE.MeshStandardMaterial) {
          ...
          const base = store.baseRoughness ?? (store.baseRoughness = mat.roughness);
          mat.envMapIntensity = 2.4;
          mat.roughness = Math.max(0.12, base * 0.55);
